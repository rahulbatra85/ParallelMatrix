...
public class LUDecomposeParallel2DBlockCyclic implements IfaceLUDecompose {
	...
	public Matrix[] LUDecompose(Matrix A) {
		...
		if(mc.getMaxThreads() <=16){
			...
			if((n/numTCols*numTCols >= 1) && (n/numTRows*numTRows >= 1)){
				wX = n/numTCols*numTCols + (n%(numTCols*numTCols)>0?1:0);
				wY = n/numTRows*numTRows + (n%(numTRows*numTRows)>0?1:0);
			} else{
		...
		//System.out.println("numTRows: " + numTRows + "numTCols: " + numTCols + "wX: " + wX + "wY: " + wY);
		...
		for(int x=0; x<numTRows; x++){
			for(int y=0; y<numTCols; y++){
				ms[x][y] = new workerThread(A, permM, x, y, wX, wY, numTRows, numTCols, msBarrier, cyclic);
	...
	class workerThread implements Runnable{
		Matrix A, p;
		int tidx, tidy;
		int wX, wY, numTCols, numTRows;
		CyclicBarrier barrier;
		boolean singular, cyclic;
		
		workerThread(Matrix A, Matrix p, int tidx, int tidy, int wX, int wY, int numTRows, int numTCols, CyclicBarrier b, boolean cyclic){
			this.A = A;
			this.p = p;
			this.tidx = tidx;
			this.tidy = tidy;
			this.numTRows = numTRows;
			this.numTCols = numTCols;
			this.wX = wX;
			this.wY = wY;
			this.barrier = b;
			this.singular = false;
		...
		public void runCyclic() {	
			...
			while(k<n && !singular){
				...
				for(int rb = 0; rb<numTRows; rb++){
					int startRow = tidx*wY + numTRows*wY*rb;
					int endRow = startRow + wY-1;
					
					for(int cb=0; cb<numTCols; cb++){
						int startCol = tidy*wX + numTCols*wX*cb;
						int endCol = startCol + wX-1;
